...
Prefer `skills/outbid/paid-fetch.js` (`paidFetch(client)`). Origin 402 → pay origin. After an origin HTML 200: fat HTML (>32KiB, client heuristic) or `{markdown:true}` → `GET https://reader.outbid.sh/scrape?url=` ($0.005; 200 is markdown JSON). JS wall still spends the $0.005; `{markdown:true}` returns 422 `needs_browser`, fat-HTML heuristic keeps origin HTML. Reader throw → origin, not `/route`. Dead/timeout → `GET https://outbid.sh/route` Accept application/json ($0.01). Copy `forward_headers`. Do not follow 302.

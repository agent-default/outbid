...
`smart-fetch.js` (`smartFetch(url, options, wallet)`): origin 402 → pay origin; after origin HTML 200, fat HTML / `{markdown:true}` → reader `$0.005` (JS wall still spends; 422 only if markdown was asked, else origin HTML). Reader down keeps origin, not `/route`. Dead/timeout → peek `/top` then `GET /route` `$0.01`. Reachable 404/5xx is returned, not hopped. `POST /bid` optional. Do not bid the reader onto `#1`.
